      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <motion.div variants={itemVariants} className="lg:col-span-2 space-y-4">
          ...
          <div className="space-y-3">
            {history.length > 0 ? (
              history.slice(0, 4).map((paper) => (
                <Link key={paper.id} to="/result" state={{ paper }} className="block">
                  <div className="glass-card p-5 rounded-2xl border border-white/5 hover:border-white/10 transition-all flex items-center justify-between group">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="font-bold group-hover:text-white transition-colors">{paper.subject}</h4>
                        <p className="text-xs text-gray-500">Semester {paper.semester} • {paper.date}</p>
                      </div>
                    </div>
                    <ArrowUpRight className="w-5 h-5 text-gray-600 group-hover:text-indigo-400 transition-all group-hover:translate-x-1 group-hover:-translate-y-1" />
                  </div>
                </Link>
              ))
            ) : (
